> **SUPERSEDED.** This document describes the first version of this project, in which
> negatives were drawn chromosome-wide and positive windows were peak-centred. Under that
> design attention appeared unfaithful (hit rate 0.359) and Integrated Gradients appeared
> clearly better (0.791). A subsequent controlled 2 x 2 study showed both figures were
> artifacts of the benchmark construction: on a dataset with region-matched negatives and
> randomly padded positive windows, attention reaches 0.955 and the two methods become
> statistically indistinguishable. See the top-level [README](../README.md).
>
> Retained for reference and for reproducing the v1 numbers, which are in `outputs_v1/`
> and `data/real_sequences_v1_chromosome_negatives.csv`.
